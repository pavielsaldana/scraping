...
if linkedin_outreach_option == "Get all conversations with connections":
    ...
    min_waiting_time = int(st.text_input("Minimum waiting time", "5", key="min_waiting_time"))
    st.write("Maximum waiting time in seconds for each conversation id lookup.")
    max_waiting_time = int(st.text_input("Maximum waiting time", "10", key="max_waiting_time"))
